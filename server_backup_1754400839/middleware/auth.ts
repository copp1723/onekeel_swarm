...
export const ownsResourceOr = (ownerIdParam: string, ...allowedRoles: Array<'admin'>) => {
  return (req: Request, res: Response, next: NextFunction) => {
    if (!req.user) {
      return res.status(401).json({ 
        error: 'Authentication required',
...
export const hasMinimumRole = (userRole: 'admin' | 'user', requiredRole: 'admin' | 'user'): boolean => {
