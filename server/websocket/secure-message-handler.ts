...
export class SecureWebSocketMessageHandler extends WebSocketMessageHandler {
  ...
  private setupSecureMessageHandlers() {
    ...
    const originalHandleMessage = this.secureWss['handleMessage'].bind(
      this.secureWss
    );

    this.secureWss['handleMessage'] = async (
      ws: any,
      message: any,
      request: any
    ) => {
      ...
      try {
        ...
        logger.error(
          'Error processing secure WebSocket message:',
          error as Error
        );
        ws.send(
          JSON.stringify({
