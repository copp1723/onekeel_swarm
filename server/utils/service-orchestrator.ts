...
export class ServiceOrchestrator {
    ...
    switch (service) {
      case 'mailgun':
        if (!config.apiKey || config.apiKey.length < 10) {
          errors.push(
            'Mailgun API key is required and must be at least 10 characters'
          );
        }
      ...
      case 'twilio':
        if (!config.accountSid || !config.accountSid.startsWith('AC')) {
          errors.push(
            'Twilio Account SID is required and must start with "AC"'
          );
        }
        if (!config.authToken || config.authToken.length < 10) {
          errors.push(
            'Twilio Auth Token is required and must be at least 10 characters'
          );
        }
      ...
      case 'openrouter':
        if (!config.apiKey || config.apiKey.length < 10) {
          errors.push(
            'OpenRouter API key is required and must be at least 10 characters'
          );
        }
        break;
  ...
  async disableService(service: string): Promise<boolean> {
    try {
      ...
      await this.serviceManager.updateServiceConfig(service, {
        enabled: false,
      });
  ...
  async getServiceStatusSummary(): Promise<any> {
    ...
    const unhealthyCount = healthValues.filter(
      h => h.status === 'unhealthy'
    ).length;
    const degradedCount = healthValues.filter(
      h => h.status === 'degraded'
    ).length;
