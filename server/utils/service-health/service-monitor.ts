...
import {
  ...
  OpenRouterHealthStatus,
} from './openrouter-health';
...
export class ServiceMonitor {
  ...
  async checkAllServices(): Promise<SystemServiceHealth> {
        ...
        try {
          ...
          if (!healthStatus.configured) {
            ...
            recommendations.push(
              `Configure ${serviceName} credentials for full functionality`
            );
    ...
    const slowestService = services.reduce((a, b) =>
      a.responseTime > b.responseTime ? a : b
    ).name;
    const fastestService = services.reduce((a, b) =>
      a.responseTime < b.responseTime ? a : b
    ).name;
    ...
    if (summary.configured < summary.total) {
      recommendations.push(
        'Some external services are not configured - system will use fallback/mock modes'
      );
    }

    if (summary.connected < summary.configured) {
      recommendations.push(
        'Some configured services are not responding - check network connectivity and credentials'
      );
    }
