...
export const leads = pgTable(
  ...
  table => {
    return {
      ...
      assignedChannelIdx: index('leads_assigned_channel_idx').on(
        table.assignedChannel
      ),
...
export const communications = pgTable(
  ...
  table => {
    return {
      ...
      campaignIdIdx: index('communications_campaign_id_idx').on(
        table.campaignId
      ),
...
export const campaignSteps = pgTable(
  ...
  table => {
    return {
      campaignIdIdx: index('campaign_steps_campaign_id_idx').on(
        table.campaignId
      ),
      stepOrderIdx: index('campaign_steps_order_idx').on(
        table.campaignId,
        table.stepOrder
      ),
    };
...
export const leadCampaignEnrollments = pgTable(
  ...
  table => {
    return {
      leadCampaignIdx: index('enrollments_lead_campaign_idx').on(
        table.leadId,
        table.campaignId
      ),
...
export const analyticsEvents = pgTable(
  ...
  table => {
    return {
      eventTypeIdx: index('analytics_events_event_type_idx').on(
        table.eventType
      ),
      leadIdIdx: index('analytics_events_lead_id_idx').on(table.leadId),
      campaignIdIdx: index('analytics_events_campaign_id_idx').on(
        table.campaignId
      ),
      createdAtIdx: index('analytics_events_created_at_idx').on(
        table.createdAt
      ),
    };
  }
...
export const featureFlags = pgTable(
  'feature_flags',
  {
    ...
    // Metadata
    category: featureFlagCategoryEnum('category')
...
export const featureFlagUserOverrides = pgTable(
  ...
  table => {
    return {
      flagUserIdx: index('feature_flag_user_overrides_flag_user_idx').on(
        table.flagId,
        table.userId
      ),
    };
...
export const leadCampaignEnrollmentsRelations = relations(
...
export type LeadCampaignEnrollment =
  typeof leadCampaignEnrollments.$inferSelect;
export type NewLeadCampaignEnrollment =
  typeof leadCampaignEnrollments.$inferInsert;
