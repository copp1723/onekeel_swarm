...
export class ConversationsRepository {
  ...
  static async findByLeadId(leadId: string) {
    return await db.select().from(conversations).where(eq(conversations.leadId, leadId));
  }
...
export class AgentDecisionsRepository {
  static async create(leadId: string, agentType: string, decision: any, reasoning?: any, metadata?: any) {
    const data: any = {
      leadId,
      agentType,
      decision,
...
export class CampaignsRepository {
  ...
  static async updateLeadStatus(campaignId: string, leadId: string, status: string) {
    const [enrollment] = await db.update(leadCampaignEnrollments)
      .set({ 
...
export class AgentConfigurationsRepository {
  ...
  static async create(data: any) {
    const [config] = await db.insert(agentConfigurations).values({
      ...data,
      createdAt: new Date(),
      updatedAt: new Date()
    }).returning();
  ...
  static async updatePerformance(id: string, metric: string) {
    ...
    const performance = config.performance || { conversations: 0, successful: 0 };
