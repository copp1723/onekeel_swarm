...
export class ExecutionProcessor {
  ...
  async processExecution(execution: CampaignExecution): Promise<void> {
    try {
      ...
      const renderedContent =
        await templateRenderingService.renderTemplateForLead(
      ...
      if (!renderedContent) {
        throw new Error(
          `Failed to render template ${execution.templateId} for lead ${execution.leadId}`
        );
      }
  ...
  private async getLeadEmail(leadId: string): Promise<{ email: string }> {
    try {
      ...
      logger.error('Failed to get lead email', {
        leadId,
        error: (error as Error).message,
      });
      throw error;
