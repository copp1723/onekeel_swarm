...
export class InviteCleanupService {
  ...
  async cleanupExpiredInvites(): Promise<{ cleaned: number; errors: number }> {
    ...
    try {
      ...
      cutoffDate.setDate(cutoffDate.getDate() - inviteConfig.cleanupExpiredAfterDays);
      
      for (const invite of invites) {
        try {
          const changes = typeof invite.changes === 'string' 
      ...
      console.log(`Cleanup completed: ${cleaned} invites cleaned, ${errors} errors`);
    ...
    try {
      ...
      for (const invite of invites) {
        try {
          ...
          if (expiresAt > now && expiresAt < reminderThreshold && !changes.reminderSent) {
            // Mark as reminder sent to avoid duplicates
            const updatedChanges = { ...changes, reminderSent: true, reminderSentAt: new Date().toISOString() };
            ...
            console.log(`Would send reminder to ${changes.email} (expires ${expiresAt.toLocaleString()})`);
            sent++;
          }
        } catch (error) {
          console.error(`Error processing reminder for invite ${invite.id}:`, error);
          errors++;
        }
      }
      
      console.log(`Reminder check completed: ${sent} reminders sent, ${errors} errors`);
...
export function scheduleInviteCleanup() {
  ...
  console.log(`Invite cleanup scheduled to run at ${next2AM.toLocaleString()} and then daily`);
}
