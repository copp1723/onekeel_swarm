...
export class LeadProcessor {
  ...
  async processNewLead(lead: any, useBackgroundJob: boolean = false) {
    if (useBackgroundJob && queueManager.isHealthy()) {
      ...
      try {
        const jobId = await queueManager.addLeadProcessingJob(
          lead.id,
          'normal'
        );
        ...
        console.warn(
          'Failed to queue lead for background processing, processing immediately',
          {
            leadId: lead.id,
    ...
    try {
      logger.info('Lead processing started', {
        leadId: lead.id,
        leadName: `${lead.firstName || ''} ${lead.lastName || ''}`.trim(),
      });
      ...
      if (lead.campaignId) {
        campaign =
          (await CampaignsRepository.findById(lead.campaignId)) || undefined;
      }
  ...
  private async processChannelAssignment(
    lead: any,
    decision: any,
    campaign: any
  ) {
    ...
    feedbackService.success(
      `Lead ${lead.firstName || ''} ${lead.lastName || ''} assigned to ${channel} channel`
    );
  }
  ...
  private async sendMessage(
    ...
    try {
      if (channel === 'email' && lead.email) {
        ...
        sendStatus = 'sent';
        logger.info('Email communication sent', {
          leadId: lead.id,
        ...
        sendStatus = 'sent';
        logger.info('SMS communication sent', {
          leadId: lead.id,
        ...
        logger.info('Chat communication initiated', {
          leadId: lead.id,
          leadName: `${lead.firstName || ''} ${lead.lastName || ''}`.trim(),
  ...
  private async handleProcessingError(lead: any, error: Error) {
    ...
    feedbackService.error(
      `Failed to process lead ${lead.firstName || ''} ${lead.lastName || ''}: ${error.message}`
    );
  }
