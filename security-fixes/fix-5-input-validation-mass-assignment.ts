...
export const paginationSchema = z.object({
  limit: z.string().regex(/^\d+$/).transform(val => Math.min(100, Math.max(1, parseInt(val)))).optional(),
  offset: z.string().regex(/^\d+$/).transform(val => Math.max(0, parseInt(val))).optional(),
  sort: z.enum(['createdAt', 'updatedAt', 'name', 'email', 'status']).optional(),
