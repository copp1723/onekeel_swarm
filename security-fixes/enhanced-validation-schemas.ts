    ...
    settings: z
      .object({
        ...
        qualificationCriteria: z.object({
          ...
          requiredFields: z
            .array(z.enum(['email', 'phone', 'name', 'company']))
            .min(1),
...
export const triggerCampaignSchema = z.object({
  ...
  leadIds: z
    .array(z.string().uuid())
    .min(1)
    .max(1000, 'Maximum 1000 leads per trigger'),
  templates: z
    .array(
      z.object({
        subject: safeString(200).regex(
          /^[^<>]*$/,
...
export const assignLeadsSchema = z.object({
  leadIds: z
    .array(z.string().uuid())
    .min(1)
    .max(5000, 'Maximum 5000 leads per assignment'),
