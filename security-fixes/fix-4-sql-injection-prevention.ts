...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const allowedSortFields = ['createdAt', 'updatedAt', 'name', 'type', 'active'];
    const validatedSort = allowedSortFields.includes(sort as string) ? sort as string : 'createdAt';
...
router.post('/execution/trigger', requireAuth, authorize('admin', 'manager'), validateRequest({ body: triggerCampaignSchema }), async (req, res) => {
  try {
...
export const sqlSanitizers = {
  ...
  validateFieldName: (field: string, allowedFields: string[]): string | null => {
