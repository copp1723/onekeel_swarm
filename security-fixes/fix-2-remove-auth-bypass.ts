...
export const authenticate = async (
  ...
  try {
    ...
    if (process.env.ENABLE_ACCESS_LOGS === 'true') {
      console.log(
        `Access: ${user.email} from ${ipAddress} at ${new Date().toISOString()}`
      );
    }
...
export const authorize = (
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (userRoleValue < minRequiredRole) {
      ...
      console.warn(
        `Authorization failed: ${req.user.email} attempted to access resource requiring ${allowedRoles.join(' or ')}`
      );
