...
const rateLimits = {
  ...
  triggerCampaign: rateLimit({
    ...
    message:
      'Campaign trigger rate limit exceeded. Please wait before triggering again.',
  ...
  emailSending: rateLimit({
    ...
    keyGenerator: (req: Request) =>
      `email_${req.params.campaignId}_${req.user?.id}`,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      dynamicLimiter.checkSuspiciousActivity(req.user.id, 'create_campaign')
    ) {
