...
export const secureFileUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!ALLOWED_FILE_TYPES.includes(ext)) {
      return cb(new Error('Invalid file type. Only CSV and TXT files are allowed.'));
    }
...
export class SecureCampaignExecutor {
  ...
  private domainSendCounts = new Map<string, { count: number; resetTime: number }>();
  ...
  async validateRecipient(email: string): Promise<boolean> {
    ...
    if (domainLimit) {
      if (now < domainLimit.resetTime && domainLimit.count >= 100) {
        logger.warn('Domain rate limit exceeded', { domain, count: domainLimit.count });
        return false;
      }
...
export class SecurityMonitor {
  ...
  private sendSecurityAlert(eventType: string, userId: string | undefined, count: number, details: any) {
