...
app.listen(PORT, () => {
  ...
  console.log(`   curl "http://localhost:${PORT}/test/sql-injection?search=' OR '1'='1"`);
  
  console.log('\n2. XSS attempt:');
  console.log(`   curl -X POST http://localhost:${PORT}/test/xss -H "Content-Type: application/json" -d '{"content":"<script>alert(1)</script>"}'`);
  
  console.log('\n3. Brute force login (run multiple times):');
  console.log(`   for i in {1..10}; do curl -X POST http://localhost:${PORT}/test/login -H "Content-Type: application/json" -d '{"username":"test@example.com","password":"wrong"}'; done`);
