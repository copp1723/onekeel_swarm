...
const envSchemas = {
  ...
  JWT_SECRET: z
    ...
    .refine(
      ...
      {
        message:
          'JWT_SECRET has insufficient entropy. Use a cryptographically secure random string.',
      }
    ),
  ...
  ENABLE_ACCESS_LOGS: z
    .enum(['true', 'false'])
    .transform(v => v === 'true')
    .default('true'),
  ENABLE_SECURITY_MONITORING: z
    .enum(['true', 'false'])
    .transform(v => v === 'true')
    .default('true'),
...
const securityChecks: SecurityCheck[] = [
  ...
  },
  {
    ...
    check: () => {
      const urls = [process.env.DATABASE_URL, process.env.REDIS_URL].filter(
        Boolean
      );
      return urls.some(
...
export class EnvironmentValidator {
  ...
  private generateMissingSecrets(): void {
    ...
    for (const key of secretKeys) {
      if (
        !process.env[key] &&
        !envSchemas[key as keyof typeof envSchemas].isOptional()
      ) {
        const secret = crypto
          .randomBytes(32)
          .toString('base64')
...
if (
  ...
  process.env.ENABLE_ENV_VALIDATION === 'true'
) {
