...
const PROD_DATABASE_URL = 'postgresql://ccl_3_user:P8LUqfkbIB4noaUDthVtZETTWZR668nI@dpg-d1lvpq6r433s73e9vfu0-a.oregon-postgres.render.com/ccl_3';
...
async function testProductionDatabase() {
  ...
  try {
    ...
    console.log(`   Size: ${(Number(dbInfo[0].size) / 1024 / 1024).toFixed(2)} MB`);
    ...
    `;
    console.log(`   campaigns.description: ${campaignsCols.length > 0 ? '✅ exists' : '❌ missing'}`);
    ...
    `;
    console.log(`   agent_configurations.context_note: ${agentCols.length > 0 ? '✅ exists' : '❌ missing'}`);
    
    // Check for feature_flags table
    const featureFlagsExists = existingTables.has('feature_flags');
    console.log(`   feature_flags table: ${featureFlagsExists ? '✅ exists' : '❌ missing'}`);
    ...
    if (missingTables.length > 0 || !campaignsCols.length || !agentCols.length || !featureFlagsExists) {
    ...
    console.log('   3. Ensure your IP is allowed (Render allows all IPs by default)');
  }
}
