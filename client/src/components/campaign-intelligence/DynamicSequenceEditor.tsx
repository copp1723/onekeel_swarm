...
export function DynamicSequenceEditor({
  sequence,
  templates,
  onSave,
}: DynamicSequenceEditorProps) {
  ...
  return (
    <div className='space-y-6'>
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-sm text-gray-600'>
            Create multi-step, AI-adaptive follow-up sequences
          </p>
        </div>
            ...
            <Card>
              ...
              <CardContent className='space-y-4'>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  ...
                  <div>
                    <Label htmlFor={`template-${touchPoint.id}`}>
                      Template
                    </Label>
                    <Select
                    ...
                    >
                      ...
                      <SelectContent>
                        {templates
                          .filter(t => t.type === touchPoint.channel)
                          .map(template => (
                  ...
                  <div className='flex gap-2'>
                    <div className='flex-1'>
                      <Label htmlFor={`delay-days-${touchPoint.id}`}>
                        Days
                      </Label>
                      <Input
                    ...
                    <div className='flex-1'>
                      <Label htmlFor={`delay-hours-${touchPoint.id}`}>
                        Hours
                      </Label>
                      <Input
                ...
                {touchPoint.delayDays > 0 || touchPoint.delayHours > 0 ? (
                  <div className='flex items-center gap-2 text-sm text-gray-600 bg-gray-50 p-2 rounded'>
                    ...
                    <span>
                      Sends{' '}
                      {touchPoint.delayDays > 0 &&
                        `${touchPoint.delayDays} day${touchPoint.delayDays > 1 ? 's' : ''}`}
                      {touchPoint.delayDays > 0 &&
                        touchPoint.delayHours > 0 &&
                        ' and '}
                      {touchPoint.delayHours > 0 &&
                        `${touchPoint.delayHours} hour${touchPoint.delayHours > 1 ? 's' : ''}`}{' '}
                      after previous step
                    </span>
                  </div>
                ) : null}
        ...
        <Card>
          <CardContent className='text-center py-8'>
            ...
            <p className='text-gray-500 mb-4'>
              Add touch points to create your campaign sequence
            </p>
      ...
      <div className='flex justify-end gap-2 pt-4 border-t'>
        ...
        <Button onClick={() => onSave(touchPoints)}>Save Sequence</Button>
      </div>
    </div>
  );
}
