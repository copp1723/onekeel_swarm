...
export function EmailScheduler() {
  ...
  const addScheduleAttempt = () => {
    const newAttempt: ScheduleAttempt = {
      ...
      templateId: '',
      delayDays:
        scheduleAttempts.length > 0
          ? scheduleAttempts[scheduleAttempts.length - 1].delayDays + 1
          : 1,
      delayHours: 0,
    };
  ...
  const updateScheduleAttempt = (
    index: number,
    updates: Partial<ScheduleAttempt>
  ) => {
                        ...
                        <div className='flex items-start justify-between'>
                          <div className='flex-1'>
                            ...
                            <p className='text-sm text-gray-500 mt-1'>
                              {schedule.attempts.length} email
                              {schedule.attempts.length !== 1 ? 's' : ''} in
                              sequence
                            </p>
                                ...
                                >
                                  ...
                                  {(attempt.delayDays > 0 ||
                                    ...
                                    <span className='text-gray-500'>
                                      • After{' '}
                                      {attempt.delayDays > 0 &&
                                        `${attempt.delayDays} day${attempt.delayDays !== 1 ? 's' : ''}`}
                                      {attempt.delayDays > 0 &&
                                        attempt.delayHours > 0 &&
                                        ' and '}
                                      {attempt.delayHours > 0 &&
                                        `${attempt.delayHours} hour${attempt.delayHours !== 1 ? 's' : ''}`}
                                    </span>
                                  )}
                                </div>
                              ))}
                    ...
                    >
                      <div>
                        ...
                        <p className='text-sm text-gray-500'>
                          {getTemplateName(email.templateId)} • Scheduled for{' '}
