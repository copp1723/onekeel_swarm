...
export function MultiAgentCampaignEditor({
  campaign,
  agents = [],
  onSave,
  onCancel,
}: MultiAgentCampaignEditorProps) {
  ...
  const addAgent = (agentId: string) => {
    ...
    setFormData(prev => ({
      ...
      assignedAgents: [
        ...
        {
          agentId,
          channels: agent.capabilities.email
            ? ['email']
            : agent.capabilities.sms
              ? ['sms']
  ...
  const removeAgent = (index: number) =>
    setFormData(prev => ({
      ...prev,
      assignedAgents: prev.assignedAgents.filter(
  ...
  const filteredAgents = availableAgents.filter(
    agent =>
      !formData.assignedAgents.some(
        (assigned: AssignedAgent) => assigned.agentId === agent.id
      )
  );
        ...
        <CardContent className='space-y-6'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={value =>
                  setFormData(f => ({ ...f, coordinationStrategy: value }))
                }
              >
          ...
          <div>
            ...
            <div className='space-y-3 mt-4'>
              ...
              {formData.assignedAgents.map(
                (assignedAgent: AssignedAgent, index: number) => {
                  const agent = availableAgents.find(
                    a => a.id === assignedAgent.agentId
                  );
                  if (!agent) return null;
                  return (
                    <div
                      key={assignedAgent.agentId}
                      className='flex flex-col md:flex-row items-start md:items-center justify-between bg-gray-50 p-3 rounded'
                    >
                      ...
                      <div className='flex flex-col md:flex-row md:items-center gap-2 mt-2 md:mt-0'>
                        ...
                        <div className='flex items-center space-x-2'>
                          {(['email', 'sms', 'chat'] as const).map(channel => (
                            ...
                            >
                              <Checkbox
                                checked={assignedAgent.channels.includes(
                                  channel
                                )}
