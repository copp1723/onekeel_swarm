...
export function LeadView({ leads, onLeadSelect, onModeSwitch }: LeadViewProps) {
  const [filterMode, setFilterMode] = useState<
    'all' | 'template' | 'ai' | 'human'
  >('all');
  const [sortBy, setSortBy] = useState<'recent' | 'engagement' | 'progress'>(
    'recent'
  );
  ...
  const filteredLeads = (leads || []).filter(
    lead => filterMode === 'all' || lead.conversationMode === filterMode
  );
  ...
  const sortedLeads = [...filteredLeads].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'engagement':
        const engagementA =
          a.metrics.emailsOpened +
          a.metrics.linksClicked +
          a.metrics.repliesReceived;
        const engagementB =
          b.metrics.emailsOpened +
          b.metrics.linksClicked +
          b.metrics.repliesReceived;
          ...
          <div className='flex items-center bg-gray-100 rounded-lg p-1'>
            ...
            >
              ...
              <span>
                Template (
                {
                  (leads || []).filter(l => l.conversationMode === 'template')
                    .length
                }
                )
              </span>
            ...
            >
              ...
              <span>
                AI (
                {(leads || []).filter(l => l.conversationMode === 'ai').length})
              </span>
            </Button>
            <Button
            ...
            >
              ...
              <span>
                Human (
                {
                  (leads || []).filter(l => l.conversationMode === 'human')
                    .length
                }
                )
              </span>
          ...
          <CardContent className='p-4'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {
                    (leads || []).filter(l => l.conversationMode === 'ai')
                      .length
                  }
                </p>
              </div>
                      ...
                      <div className='space-y-2'>
                        <div className='flex items-center justify-between text-sm'>
                          <span className='text-gray-600'>
                            Email {lead.templateProgress.current} of{' '}
                            {lead.templateProgress.total}
                          </span>
                          <span className='text-gray-500'>
                            {Math.round(
                              (lead.templateProgress.current /
                                lead.templateProgress.total) *
                                100
                            )}
                            % complete
                        ...
                        <div className='flex items-center justify-between'>
                          <div className='space-y-1'>
                            ...
                            <p className='text-xs text-purple-700'>
                              {lead.aiEngagement.messagesExchanged} messages
                              exchanged
                            </p>
                          </div>
                          <div className='flex items-center space-x-2'>
