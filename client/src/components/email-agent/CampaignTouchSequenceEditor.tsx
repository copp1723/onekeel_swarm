...
export const CampaignTouchSequenceEditor: React.FC<
  CampaignTouchSequenceEditorProps
> = ({ sequence, onChange }) => {
  ...
  const addStep = () => {
    onChange([...sequence, { templateId: '', delayDays: 0, delayHours: 0 }]);
  };
    ...
    <div>
      ...
      <div className='space-y-4'>
        {sequence.map((step, idx) => (
          <div
            key={idx}
            className='flex gap-2 items-center border rounded p-2 bg-muted'
          >
            <input
              type='text'
            ...
            <input
              ...
              onChange={e =>
                updateStep(idx, { delayHours: parseInt(e.target.value) || 0 })
              }
              placeholder='Hours'
            />
            <button
