...
export const ReviewStep: React.FC<{ ctx: WizardContext; agents: Agent[] }> = ({
  ctx,
  agents,
}) => {
  return (
    <div className='space-y-4'>
      ...
      <div className='space-y-3'>
        ...
        <div className='p-3 bg-gray-50 rounded'>
          ...
          <p className='text-sm'>
            {agents.find((a: Agent) => a.id === ctx.data.agentId)?.name ||
              'No agent selected'}
          </p>
        </div>
        <div className='p-3 bg-gray-50 rounded'>
          ...
          <div className='text-sm space-y-1'>
            <p>
              ...
              {ctx.data.offer.product || 'Not set'}
            </p>
            <p>
              <span className='font-medium'>Pricing:</span>{' '}
              {ctx.data.offer.pricing || 'Not set'}
            </p>
            <p>
              <span className='font-medium'>CTA:</span>{' '}
              {ctx.data.offer.cta.primary || 'Not set'}
            </p>
          </div>
        </div>
        <div className='p-3 bg-gray-50 rounded'>
        ...
        <div className='p-3 bg-gray-50 rounded'>
          ...
          <p className='text-sm'>
            {ctx.data.schedule.totalEmails} emails,{' '}
            {ctx.data.schedule.daysBetweenEmails} days apart
          </p>
