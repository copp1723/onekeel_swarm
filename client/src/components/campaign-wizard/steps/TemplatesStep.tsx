...
export const TemplatesStep: React.FC<{ ctx: WizardContext }> = ({ ctx }) => {
  const [selectedTab, setSelectedTab] = useState<'generate' | 'library'>(
    'generate'
  );
  ...
  const handleTemplatesFromLibrary = (templates: any[]) => {
    ...
    const campaignTemplates: EmailTemplate[] = templates.map(
      (template, index) => ({
        id: template.id,
    ...
    <div className='space-y-4'>
      <div className='bg-purple-50 p-4 rounded-lg'>
        ...
        <p className='text-sm text-purple-700'>
          Choose how to create your campaign templates - generate with AI or
      ...
      <Tabs
        ...
        onValueChange={(value: string) =>
          setSelectedTab(value as 'generate' | 'library')
        }
      >
                ...
                <div className='flex items-start space-x-2'>
                  ...
                  <div className='text-sm'>
                    ...
                    <p className='text-blue-700 mt-1'>
                      AI will create {ctx.data.schedule.totalEmails} progressive
                      emails based on your offer, each with unique subject lines
                      and escalating urgency. When a lead replies to ANY email,
                      the remaining templates are cancelled and AI takes over
                      for personalized conversation.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className='space-y-4'>
              <div className='flex items-center justify-between'>
                <h4 className='font-medium'>
                  Generated Templates ({ctx.data.templates.length})
                </h4>
                <Button
                  variant='outline'
                  size='sm'
                      ...
                      <CardHeader className='pb-2'>
                        <div className='flex items-center justify-between'>
                          ...
                          <Badge variant='outline'>
                            Day{' '}
                            {index * ctx.data.schedule.daysBetweenEmails + 1}
                          </Badge>
                        </div>
                      </CardHeader>
        ...
        <TabsContent value='library' className='space-y-4'>
          <div className='flex items-center justify-between mb-4'>
            ...
            <Button variant='outline' size='sm' onClick={openTemplateLibrary}>
            ...
            <div className='mt-4 p-3 bg-green-50 rounded-lg'>
              <p className='text-sm text-green-800'>
                <span className='font-medium'>
                  {ctx.data.templates.length} templates
                </span>{' '}
        ...
        <div className='mt-6 p-4 bg-gray-50 rounded-lg'>
          ...
          <div className='space-y-2 text-sm'>
            ...
            <div className='flex items-center justify-between'>
              ...
              <span className='font-medium'>
                {(ctx.data.templates.length - 1) *
                  ctx.data.schedule.daysBetweenEmails}{' '}
                days
              </span>
            </div>
            <div className='flex items-center justify-between'>
