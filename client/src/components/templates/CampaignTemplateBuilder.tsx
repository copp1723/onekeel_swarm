...
export function CampaignTemplateBuilder({
  availableTemplates,
  onSave,
}: CampaignTemplateBuilderProps) {
  const [name, setName] = useState('');
  const [industry, setIndustry] = useState('');
  const [sequence, setSequence] = useState<
    { templateId: string; delayDays: number }[]
  >([]);
          ...
          {sequence.map((item, index) => (
            ...
            >
              <p className='flex-grow'>
                {availableTemplates.find(t => t.id === item.templateId)?.name ||
                  'Unknown Template'}
              </p>
              <Input
                type='number'
