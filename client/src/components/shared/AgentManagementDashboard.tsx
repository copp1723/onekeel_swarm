...
export function AgentManagementDashboard({
  ...
  const [editingAgent, setEditingAgent] = useState<UnifiedAgentConfig | null>(
    null
  );
  const [selectedAgent, setSelectedAgent] = useState<UnifiedAgentConfig | null>(
    null
  );
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    const matchesPersonality =
      ...
      (typeof agent.personality === 'object' && agent.personality
        ? agent.personality.style === filterPersonality
        : agent.personality === filterPersonality);
    const matchesActive =
      filterActive === 'all' ||
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-gray-600'>
              Detailed view and configuration for {selectedAgent.name}
            </p>
          </div>
                  ...
                  <p className='text-2xl font-bold'>
                    {
                      Object.keys(stats.byType).filter(
                        type => stats.byType[type as AgentType] > 0
                      ).length
                    }
                  </p>
                </div>
          ...
          <div className='flex flex-col md:flex-row gap-4'>
            ...
            <div className='flex gap-2'>
              <Select
                value={filterType}
                onValueChange={value =>
                  setFilterType(value as AgentType | 'all')
                }
              >
          ...
          {filteredAgents.length === 0 ? (
            ...
            filteredAgents.map(agent => (
              <AgentCard
                ...
                onViewAnalytics={
                  showAgentDetails ? handleViewAgentDetails : onAgentSelect
                }
                compact={compact}
              />
            ))
