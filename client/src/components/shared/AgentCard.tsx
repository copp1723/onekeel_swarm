...
export function AgentCard({
  ...
  const getCapabilityIcons = () => {
    ...
    if (agent.capabilities?.email)
      icons.push(<Mail key='email' className='h-3 w-3' />);
    if (agent.capabilities?.sms)
      icons.push(<Phone key='sms' className='h-3 w-3' />);
    if (agent.capabilities?.chat)
      icons.push(<MessageSquare key='chat' className='h-3 w-3' />);
    return icons;
  };
        ...
        <CardContent className='p-4'>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center space-x-2'>
              <div className='flex space-x-1'>{getCapabilityIcons()}</div>
    ...
    <Card className='hover:shadow-lg transition-all duration-200 border-l-4 border-l-blue-500'>
      <CardHeader className='pb-3'>
        <div className='flex items-start justify-between'>
          ...
          <div className='flex items-center space-x-2'>
            <div
              className={`w-3 h-3 rounded-full ${agent.active ? 'bg-green-500' : 'bg-gray-400'}`}
            />
