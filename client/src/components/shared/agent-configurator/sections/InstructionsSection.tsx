              ...
              {instructions.dos.map((instruction: string, index: number) => (
                <div key={index} className='flex items-center space-x-2'>
                  <Input
                    value={instruction}
                    onChange={e =>
                      updateInstruction('dos', index, e.target.value)
                    }
              ...
              {instructions.donts.map((instruction: string, index: number) => (
                <div key={index} className='flex items-center space-x-2'>
                  <Input
                    value={instruction}
                    onChange={e =>
                      updateInstruction('donts', index, e.target.value)
                    }
        ...
        {(instructions.dos.length > 0 || instructions.donts.length > 0) && (
          <div className='bg-gray-50 p-3 rounded-lg'>
            <p className='text-sm text-gray-700'>
              <strong>Summary:</strong> {instructions.dos.length} do's and{' '}
              {instructions.donts.length} don'ts defined
            </p>
          </div>
        )}
      </CardContent>
