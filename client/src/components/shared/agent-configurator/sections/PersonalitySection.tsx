...
export function PersonalitySection({ formData, setFormData }: SectionProps) {
  ...
  const getCurrentPersonality = () => {
    if (
      typeof formData.personality === 'object' &&
      formData.personality?.style
    ) {
      ...
      <CardContent className='space-y-4'>
        ...
        <div className='bg-blue-50 p-3 rounded-lg'>
          <p className='text-sm text-blue-700'>
            ...
            {
              PERSONALITY_OPTIONS[
                getCurrentPersonality() as keyof typeof PERSONALITY_OPTIONS
              ]
            }{' '}
            •<strong> Tone:</strong>{' '}
            {TONE_OPTIONS.find(t => t.value === formData.tone)?.label ||
              'Friendly'}{' '}
            •<strong> Length:</strong>{' '}
            {RESPONSE_LENGTH_OPTIONS.find(
              r => r.value === formData.responseLength
            )?.label || 'Moderate'}
          </p>
        </div>
      </CardContent>
