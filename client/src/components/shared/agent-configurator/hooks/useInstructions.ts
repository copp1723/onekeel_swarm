...
export function useInstructions(
  ...
  const updateInstruction = useCallback(
    (type: 'dos' | 'donts', index: number, value: string) => {
      setFormData(prev => {
        const currentInstructions = getInstructionsAsObject(prev.instructions);
        return {
          ...prev,
    ...
    (type: 'dos' | 'donts', index: number) => {
      setFormData(prev => {
        ...
        return {
          ...
          instructions: {
            ...currentInstructions,
            [type]: currentInstructions[type].filter(
   ...
   */
  const setInstructionsFromTemplate = useCallback(
    (template: InstructionsData) => {
      setFormData(prev => ({
        ...prev,
