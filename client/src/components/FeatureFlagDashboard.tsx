...
class FeatureFlagAdminService {
  ...
  static async toggleFlag(flagKey: string, enabled: boolean): Promise<void> {
    ...
    const response = await fetch(
      `/api/feature-flags/admin/${flagKey}/${endpoint}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className='bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow'>
      ...
      <div className='flex items-start justify-between mb-4'>
        ...
        <label className='relative inline-flex items-center cursor-pointer ml-4'>
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              onToggle(flag.key, e.target.checked)
            }
      ...
      <div className='flex flex-wrap gap-2 mb-4'>
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${getCategoryColor(flag.category)}`}
        >
          {flag.category}
        </span>
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${getComplexityColor(flag.complexity)}`}
        >
          {flag.complexity}
        </span>
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${getRiskColor(flag.riskLevel)}`}
        >
          {flag.riskLevel} risk
        </span>
      </div>
...
export const FeatureFlagDashboard: React.FC = () => {
  ...
  const categories = [
    'all',
    ...Array.from(new Set(flags.map((f: FeatureFlag) => f.category))),
  ];
      ...
      <div className='grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8'>
        ...
        <div className='bg-white p-4 rounded-lg border border-gray-200'>
          <div className='text-2xl font-bold text-purple-600'>
            {
              flags.filter((f: FeatureFlag) => f.complexity === 'advanced')
                .length
            }
          </div>
