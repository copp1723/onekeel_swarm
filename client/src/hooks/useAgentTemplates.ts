...
export function useAgentTemplates() {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      const grouped = data.templates.reduce(
        (acc: Record<string, AgentTemplate[]>, template: AgentTemplate) => {
  ...
  const createAgentFromTemplate = useCallback(
    async (id: string, name: string, paramValues: Record<string, string>) => {
      setLoading(true);
      setError(null);

      try {
        const response = await fetch(
          `/api/agent-templates/${id}/create-agent`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
